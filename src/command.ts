...
import type { EventType, NotifierConfig } from "./config.js";
...
export function runCommand(
  ...
  const args = (config.command.args ?? []).map((arg) =>
    substituteTokens(arg, event, message, sessionTitle, agentName, projectName, timestamp, turn),
  );
  const command = substituteTokens(
    config.command.path,
    event,
    message,
    sessionTitle,
