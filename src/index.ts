...
import { shouldSuppressPermissionAlert, prunePermissionAlertState } from "./permission-dedupe.js";
...
async function handleEvent(
  ...
  if (config.webhook.enabled && config.webhook.targets.length > 0 && isEventWebhookEnabled(config, eventType)) {
    const title = getNotificationTitle(config, projectName);
    const eventOverrides = config.webhook.events?.[eventType as keyof typeof config.webhook.events];
    const finalMessage = eventOverrides?.message
      ? interpolateMessage(eventOverrides.message, {
          sessionTitle: config.showSessionTitle ? sessionTitle : null,
          agentName,
          projectName,
          timestamp,
    ...
    sendWebhook(config.webhook.targets, title, finalMessage, eventType, eventOverrides);
  }
...
export const WebhookNotifierPlugin: Plugin = async ({ client, directory }) => {
  ...
  return {
    event: async ({ event }) => {
      ...
      if (event.type === "session.status" && (event.properties as any)?.status?.type === "busy") {
        markSessionBusy((event.properties as any).sessionID);
      }

      if (event.type === "session.error") {
        const sessionID = getSessionIDFromEvent(event);
        markSessionError(sessionID);
        const errorName = (event.properties as any)?.error?.name;
        const eventType: EventType = errorName === "MessageAbortedError" ? "user_cancelled" : "error";
